...
The reproducible lanes install under a lock; the canary does not. Every job in `test-suite.yml`, plus the compat matrix's promised lanes (including `snippets-lts`), sets `PIP_CONSTRAINT` to a committed pip constraints file (`requirements/ci-py311.txt`, and one `requirements/compat-py3.NN.txt` per matrix interpreter). Those files pin the environment without touching the ranges in `pyproject.toml`, so a promised lane cannot go red because of what PyPI published that morning. The canary sets no constraint on purpose: resolving fresh is what makes it the early warning. Regenerate the files with `bash scripts/lock.sh` (see CONTRIBUTING.md).
...
| `duckdb` | `channel.field.metrics_boundary` | unsupported | — | See feature.metrics_boundary — DuckDB has no stage-fusion concept for this key to configure a boundary around. |
| `duckdb` | `channel.field.num` | unsupported | — | op: repartition/coalesce is unimplemented on DuckDB — see channel.op.repartition / channel.op.coalesce. |
| `duckdb` | `channel.field.num_partitions` | unsupported | — | op: repartition is unimplemented on DuckDB (no partition concept) — see channel.op.repartition. |
| `duckdb` | `channel.field.storage_level` | unsupported | — | op: cache is unimplemented on DuckDB (single-process, no distributed cache tier) — see channel.op.cache. |
...
| `duckdb` | `egress_maintenance.field.optimize` | unsupported | — | DuckDB has no OPTIMIZE/compaction operation for the formats it writes. Not implemented — see egress.field.maintenance. |
| `duckdb` | `egress_maintenance.field.rewrite_data_files` | unsupported | — | Iceberg maintenance procedures require an Iceberg catalog DuckDB does not write through. Not implemented — see egress.field.maintenance. |
| `duckdb` | `egress_maintenance.field.vacuum` | unsupported | — | DuckDB has no VACUUM/cleanup operation for the formats it writes. Not implemented — see egress.field.maintenance. |
| `duckdb` | `egress_maintenance.field.zorder_by` | unsupported | — | DuckDB has no OPTIMIZE/ZORDER operation for the formats it writes. Not implemented — see egress.field.maintenance. |
...
| `duckdb` | `feature.metrics_boundary` | unsupported | — | metrics_boundary works around Spark's stage-fusion by forcing a shuffle boundary so SparkListener attributes metrics per module — a fix for a problem specific to that collection mechanism. DuckDB has no stage-fusion concept to work around; this engine's executor writes per-module module_metrics directly after each dispatch branch (see observability.module_metrics.per_module), with no fusion/attribution ambiguity for this flag to configure either way. |
...
| `duckdb` | `tooling.doctor.cloud_preflight` | unsupported | — | No DuckDB object-existence preflight exists — aqueduct doctor --preflight's cloud-URI check unconditionally builds a SparkSession and probes via Spark's Hadoop FileSystem using engine.spark.conf credentials, even for a module that resolves to duckdb (which would need engine.duckdb.s3_*/httpfs instead). A DuckDB-native probe using duckdb_/extensions.py's ensure_extension + httpfs is not built yet; until it is, --preflight gives no coverage for duckdb-resolved sources. |
| `duckdb` | `tooling.doctor.session_preflight` | unsupported | — | There is no check_duckdb()/session-preflight analog to check_java()+check_spark(preflight=True) — DuckDB session/extension/S3-secret verification only happens incidentally inside _probe_duckdb_handoff, gated on handoff.root being remote with credentials configured. A blueprint whose DuckDB Ingress/Egress modules read/write s3:// while handoff.root stays local gets zero doctor-time verification that httpfs can install/load; that failure only surfaces mid-run. Not built yet. |
| `duckdb` | `tooling.doctor.table_exists` | unsupported | — | No DuckDB table-existence probe exists — aqueduct doctor's check unconditionally builds a SparkSession and calls spark.catalog.tableExists(), with no branch for a duckdb-resolved module, and reports pyspark-not-installed rather than 'not implemented for duckdb' when it fails. A real check would be an information_schema.tables query over the project's engine.duckdb.* connection; not built yet. |
| `duckdb` | `tooling.test_runner` | unsupported | — | There is no DuckDB test runner — aqueduct/executor/duckdb_ has no test_runner.py at all. aqueduct test's test_cmd (aqueduct/cli/project.py) unconditionally imports aqueduct.executor.spark.test_runner/make_spark_session regardless of deployment.engine, so today it runs module unit tests against Spark's SQL/type semantics even for a duckdb-deployed project. Until a native DuckDB test runner ships, treat `aqueduct test` results for a DuckDB project as Spark-semantics results, not DuckDB-accurate ones. |
