...
`agent.max_patches` (default `1`, Blueprint-only) is the ONE counter for the whole heal: a single-attempt heal at the default, or, with `max_patches: N` (N > 1, `agent.approval: auto`, non-cascade path), chained multi-patch healing, the standard (and only) behavior of the multi-patch loop. There is no separate opt-in flag.
...
**Heal-as-PR.** `aqueduct patch pr <patch_ref>` branches, applies, commits, pushes, and opens a PR in one command, approval-mode-agnostic, as an alternative to the manual webhook-plus-import flow above. See [CLI Reference](../cli_reference.md#5-patch-management).
