...
| `aqueduct patch preview <file>` | Review changes and run gates. Renders the Blueprint diff, the lineage gate, the engine-config gate (the effective session-config delta the patch produces, or `not_applicable` when it writes no engine config), and with `--sandbox` the sandbox + explain gates. `aqueduct.yml` is loaded on every invocation, not only under `--sandbox`, because the engine-config gate compares against its `engine.<name>` layer; a config error exits `CONFIG_ERROR`. `--format json` adds an `engine_config` object (`status`, `detail`, `delta`, `write_targets`, `cli_pinned`). `-s PATH=VALUE` (same grammar as `aqueduct run`, repeatable) pins the resolved config the engine-config gate and `--sandbox` replay measure against — a `--set` on the SAME key the patch writes makes the write inert (`cli_pinned` non-empty in JSON; text mode prints `pinned by --set`), and the guardrails gate refuses it rather than reporting a delta this invocation cannot exhibit. |
