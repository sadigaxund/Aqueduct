...
A remote `handoff.root` (`s3://`, `gs://`, `abfss://`, …) needs `fsspec` installed (e.g. `aqueduct-core[object-store]`) — without it the command refuses cleanly rather than silently no-op'ing, same contract every other handoff-adjacent surface (`aqueduct run`, `aqueduct doctor`) follows.
