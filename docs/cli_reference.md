...
- **`aqueduct doctor`**: Live connectivity checks (Spark, stores, agent, sources). Use before deploying. Includes a `gh` check (`gh --version` + `gh auth status`) for `aqueduct patch pr`'s transport; soft/skippable when `gh` is absent, since it is only needed by that one command.
...
| `aqueduct doctor -s PATH=VALUE` | Same `-s/--set` grammar as `aqueduct run` (repeatable, in-memory, never persisted). Every connectivity check (Spark, handoff, secrets, agent, …) probes under the SAME resolved config a `--set`-pinned `aqueduct run` would use. **Exempt:** the healed-engine-config drift check (`healed-config:<patch_id>`) always compares against the UNPINNED resolution, so a `--set` on a healed key cannot manufacture or mask drift for that one row. See `aqueduct patch revert` below, which enforces the identical rule as a hard refusal rather than a warning. |
...
| `--ctx KEY=VALUE` | n/a | Override a Tier 0 context variable. Repeatable. Environments that can't pass CLI args (CI, Airflow) can set `AQUEDUCT_CTX_<KEY>` env vars instead, top-level keys only, one priority step below `--ctx` (see specs/03-resolution.md §5.2). |
...
`agent.approval` is the config key. Values: `disabled`, `human`, `auto`.
...
| `aqueduct patch preview <file>` | Review changes and run gates. Renders the Blueprint diff, the lineage gate, the engine-config gate (the effective session-config delta the patch produces, or `not_applicable` when it writes no engine config), the resolvability gate (Gate 4, `not_applicable` unless the patch carries a `declare_dependency` op, otherwise `pass`/`warn`/`fail`/`unavailable` per §8.2 of specs/06-healing.md), and with `--sandbox` the sandbox gate. `aqueduct.yml` is loaded on every invocation, not only under `--sandbox`, because the engine-config gate compares against its `engine.<name>` layer; a config error exits `CONFIG_ERROR`. `--format json` adds an `engine_config` object (`status`, `detail`, `delta`, `write_targets`, `cli_pinned`) and a `resolvability` object (`status`, `detail`, `requirements`, `duration_ms`). `-s PATH=VALUE` (same grammar as `aqueduct run`, repeatable) pins the resolved config the engine-config gate and `--sandbox` replay measure against. A `--set` on the SAME key the patch writes makes the write inert (`cli_pinned` non-empty in JSON; text mode prints `pinned by --set`), and the guardrails gate refuses it rather than reporting a delta this invocation cannot exhibit. **Exit code:** non-zero (`DATA_OR_RUNTIME`) when the lineage gate fails, the resolvability gate reports `fail`, or (under `--sandbox`) the sandbox gate blocks. A resolvability `warn` (requirement resolves but isn't installed yet, a defer, not a rejection) does **not** fail the command. |
| `aqueduct patch apply <file>` | Apply a patch |
| `aqueduct patch revert <patch_id> --blueprint <file> [--patches-dir <dir>] [--config <file>] [--dry-run] [--format text\|json] [-s PATH=VALUE]` | Undo an applied heal patch's engine-config writes, in place, without git. Restores every `engine.<name>` key the patch wrote to the value the patch index's `engine_config_delta` recorded before it was applied, then stamps the Blueprint's `healed_by:` record `reverted_at:` (the record is kept, so the heal stays in the history). Refuses loudly when the observability store holding that delta is unreachable, since an empty read there is indistinguishable from "nothing was recorded". Backs the Blueprint up under `<patches-dir>/backups/` first. Only engine-config writes can be reverted: they are the only change for which a prior value is recorded. The command REFUSES, with the reason named and nothing written, when the patch also carries a non-config operation, when a later patch overwrote the same key (revert in reverse order, or use `rollback`), when the value has been edited since, or when the plan cannot be shown to reproduce the recorded pre-patch config. Exit `DATA_OR_RUNTIME` on refusal. `--dry-run` plans and verifies without writing. `-s PATH=VALUE` (repeatable) reaches this command's own config resolution (e.g. `-s stores.blob.backend=s3` to read the applied patch body from a different patch store) but **never** the prior-values equality check above: that check always compares against the UNPINNED resolution, so a `--set` on a healed key can neither make a clean revert abort nor let a genuinely diverged one falsely pass. |
| `aqueduct patch import <file> --blueprint <bp> [--no-commit]` | Apply a received patch and `git commit` it in one step, the entry point a CI runner calls after the `on_patch_pending` webhook. Equivalent to `apply` + `commit`; `--no-commit` stages only. |
| `aqueduct patch pr <patch_ref> --blueprint <bp> [--patches-dir <dir>] [--config <file>] [--dry-run]` | Apply a staged patch on a fresh branch and open a PR for review (2.2.0, heal-as-PR). Approval-mode-agnostic: works on any pending patch regardless of the Blueprint's `approval` mode, so a `human`-mode reviewer can use it instead of a local `patch apply`, or a CI runner can use it instead of (or alongside) the webhook flow. Flow: repo-root pre-flight guard (`git:` in `aqueduct.yml`), `git switch -c aqueduct/heal/<patch_id>`, apply + commit through the identical path `patch import` uses (same gates, same `---aqueduct---` trailer), `git push -u <remote> <branch>`, then `gh pr create` (title/base/draft from `pr:` in `aqueduct.yml`). Requires the `gh` CLI, authenticated (`aqueduct doctor` checks this). `--dry-run` runs every check and prints the plan; touches nothing. On any failure the branch is never rewritten or deleted; the command reports what state the repo is in and how to continue by hand, switching HEAD back to the branch you started on (the one automatic rollback it performs). GitHub-only in 2.2.0 (`gh` CLI shellout, no provider abstraction shipped). |
...
Cross-engine `handoff.root` (the spill directory a polyglot Blueprint's upstream island materializes to and the downstream island reads back, see [specs/07-stores-and-ops.md §10.9](specs/07-stores-and-ops.md#109-engines-and-the-capability-framework)) is swept automatically at the start of every polyglot run. `aqueduct handoff sweep` is the explicit, on-demand counterpart, for a Blueprint that failed and was never rerun (its spill would otherwise sit there forever), or for routine disk hygiene between runs.

| Command | Description |
|---------|-------------|
| `aqueduct handoff sweep [--config <file>] [--store-dir <dir>]` | List (dry-run, the default) or remove orphaned spill directories under `handoff.root`. A directory is orphaned when its `run_id` is a TERMINAL row in `run_records` and not a kept failure still awaiting a resume: a non-terminal run (no `finished_at`, still running, or crashed before writing one) is NEVER touched; a terminal failure stays under `handoff.keep_on_failure` until a LATER run of the same Blueprint succeeds; a `run_id` with no `run_records` row at all is treated as unknown and reclaimed. `--store-dir` discovers every observability store found there (a shared `handoff.root` can carry spill from more than one Blueprint), not just one, so a run belonging to a Blueprint this invocation didn't explicitly name is still recognised as live. |
| `aqueduct handoff sweep --dry-run \| --execute` | `--dry-run` (the default) deletes nothing and lists every candidate with its reason. `--execute` performs the deletion. The safe default is deliberate: this removes materialized data with no undo (recomputing a lost spill re-runs a full upstream island), so the default posture always shows what would go before anything goes. |
| `aqueduct handoff sweep --format json` | `{root, dry_run, candidates: [{path, manifest_hash, run_id, status, reason, reclaimed_by_age, bytes, removed}]}`: ANSI-free, parseable. |
| `aqueduct handoff sweep --older-than <duration>` | Additionally reclaims a kept-failure spill whose run FINISHED more than `<duration>` ago, even though no later success has superseded it yet: an explicit operator/watchdog action for a Blueprint that failed and is never going to be rerun. `<duration>` is an integer plus `d`/`h`/`m` (e.g. `7d`, `24h`, `90m`); a bad value is a config error. Omitting the flag leaves `aqueduct handoff sweep` byte-identical to today. Applies to both `--dry-run` and `--execute`; reclaimed-by-age candidates carry `reclaimed_by_age: true` in `--format json` and a distinct `reason` in the text table. |

A remote `handoff.root` (`s3://`, `gs://`, `abfss://`, …) needs `fsspec` installed (e.g. `aqueduct-core[object-store]`). Without it the command refuses cleanly rather than silently no-op'ing, the same contract every other handoff-adjacent surface (`aqueduct run`, `aqueduct doctor`) follows.

---

## 8. Engine authoring (`aqueduct dev`)

Tools for writing an execution engine. An engine registers through the `aqueduct.engines` entry-point group and cannot register until its `capabilities.yml` gives an explicit verdict for every leaf on ITS OWN checklist (Spark's is 206 today: 189 Blueprint-grammar leaves + 17 engine-scoped `config.*` leaves; a `config.*` leaf that runs only in core code paths, e.g. webhooks/secrets/stores/lineage, is never on any engine's checklist at all; see [specs/07-stores-and-ops.md §10.9](specs/07-stores-and-ops.md) "Config-leaf scoping"). These commands generate and maintain that file.
