...
|distinct\_count|sample(frac) + approx\_count\_distinct()|Low — approximate|Approximate|
|data\_freshness|df.select(max(column)).collect()|1 action (full scan by default)|Exact (or approx if sampled)|
