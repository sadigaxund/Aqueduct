...
|**watermark\_column**|Required when `materialize: incremental`. Column name used to track the high-water mark (typically a timestamp or monotonic integer). The executor runs `MAX(watermark_column)` on the output DataFrame after each successful run to advance the watermark. **Cost:** this is an extra Spark action — if the output DataFrame is not cached, Spark re-executes the full DAG to compute the max (a second scan of the output data). Add a Checkpoint upstream or cache the Channel output to avoid the double scan. See [docs/diagnostics.md#incremental-watermark-scan](diagnostics.md#incremental-watermark-scan).|
