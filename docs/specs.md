...
**Version 2.36: Reference Document**
...
**Not yet wired into `aqueduct run`.** `run_polyglot()` is a complete, independently-tested runtime unit; routing the existing single-engine CLI run loop (healing, patch gates, retries, hooks, sandbox replay, run-header rendering) through it is separate follow-up integration work, tracked in `docs/roadmap.md`.
