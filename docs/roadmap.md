...
When write tools ship, `approval` in the agent config applies to the tool caller: `auto` approves patches immediately, and `human` holds them for the user to confirm in the MCP client UI. Non-stdio transports (network/SSE) are similarly deferred; the current server is deliberately local-only.
...
**Status:** Conceptual: only the domain framing exists, without a grammar, gates, or code, so a future domain slots into the same shape instead of a one-off
