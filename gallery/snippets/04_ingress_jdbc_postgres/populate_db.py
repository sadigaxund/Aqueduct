...
def main():
    ...
    if "jdbc:sqlite:" in jdbc_url:
        ...
        cursor.execute(
            "CREATE TABLE inventory (item_id INTEGER PRIMARY KEY, quantity INTEGER, last_updated TEXT)"
        )
        cursor.executemany(
        ...
        spark = (
            SparkSession.builder.appName("PopulateJDBC")
        ...
        df.write.format("jdbc").options(
            url=jdbc_url, dbtable="inventory", user=user, password=password, driver=driver
        ).mode("overwrite").save()
