...
def main():
    ...
    try:
        if product_output.exists():
            rows = con.execute(
                f"SELECT * FROM read_parquet('{product_output}') ORDER BY id"
            ).fetchall()
            ...
            console.print(t)
            console.print(
                f"  [dim]Row count: {len(rows)} — Java UDF enrichment runs in parallel with Component B[/dim]\n"
            )

        if customer_output.exists():
            rows = con.execute(
                f"SELECT * FROM read_parquet('{customer_output}') ORDER BY customer_id"
            ).fetchall()
            columns = [desc[0] for desc in con.description]
            ...
            console.print(t)
            console.print(
                f"  [dim]Row count: {len(rows)} — runs concurrently with Component A[/dim]"
            )
    finally:
        con.close()
