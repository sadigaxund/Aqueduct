...
def main():
    console.print(
        "[bold cyan]·[/bold cyan] Running pipeline (expecting ~10s wait due to Regulator timeout)...\n"
    )
    ...
    if result.returncode == 0:
        ...
        if duration >= 10:
            console.print(
                "[bold cyan]✓[/bold cyan] The Regulator correctly waited for the configured timeout!"
            )
        else:
            console.print(
                "[bold yellow]⚠[/bold yellow] Pipeline finished faster than expected — check the timeout setting."
            )
    else:
