...
### 3.1 Missing `runs` Command
*   **Findings**: `aqueduct runs` is a core command used to list history (implemented in `cli.py` and mentioned in `README`), but it is completely missing from the `specs.md` CLI Reference (§11).
*   **Action**: Add `aqueduct runs` to the formal specification.
...
| **Standardize Enums** | Low | `docs/specws.md`, `aqueduct/surveyor/models.py` |
