...
- **`aqueduct patch list` now reads the configured patch store, and the patch index stays in sync on remote backends.** `patch list` scanned the local `<blueprint-dir>/patches/` directory via `glob`, so with a non-local `stores.blob` backend (s3/gcs/adls) — where self-healing stages the patch body — it reported "No pending patches" even though one was pending. It now lists the **patch store directly** (local *or* object backend) via `PatchStore.iter_payloads` — the source of truth — resolving the backend from `aqueduct.yml` (`--config` / CWD) **plus `.env`** (so configs/blueprints that reference env or `@aq.secret()` resolve instead of silently failing to the local dir). The same `.env` fix in `_patch_index_obs_store` means the lifecycle commands' best-effort `patch_index` status updates (`apply`/`reject`/`commit`/`discard` → `set_status`) actually reach a remote obs store, keeping the index consistent. (`aqueduct/cli/patch.py`) `aqueduct patch apply` now also accepts a bare **`patch_id`** (not just a local file) and **auto-fetches the body from the configured store** — so the remote workflow is `patch list` → `patch apply <id> --blueprint …` with no manual `patch pull`. **Still local-body-bound for remote stores:** `discard`/`reject`/`commit`/`import` operate on local files (use `patch pull <id>` first); their by-`patch_id` forms are tracked in `TODOs.md`.
