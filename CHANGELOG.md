...
- **Module failure no longer renders the error three times (ISSUE-046).** A Spark `AnalysisException` was printed (1) as Spark's internal `SQLQueryContextLogger` JSON dump of the whole Catalyst plan tree to stderr, (2) from the executor's retry log with the full plan tree, and (3) on the module status line — burying the one-line root cause. Now: the executor log + status line use a `concise_error()` helper (first line / root cause only — the full text is still stored in `ModuleResult.error` for the obs store + heal agent), and on Spark 4.0 the session now defaults `spark.log.structuredLogging.enabled=false` (overridable) so Spark stops emitting JSON-structured logs that fight Aqueduct's own `--log-format`, with a best-effort JVM mute of `SQLQueryContextLogger` on top.
