...
- **Timeout hint pointed at a non-existent `--timeout` flag.** The agent-call timeout hint suggested `--timeout 600`, which is not a CLI option; it now says `--set agent.timeout=600` (or `agent.timeout:` in aqueduct.yml) and is trimmed to a single line. (`aqueduct/agent/providers.py`)
