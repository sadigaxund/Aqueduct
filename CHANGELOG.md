...
- **`aqueduct patch list` now sees patches staged to a remote object store.** It scanned the local `<blueprint-dir>/patches/` directory via `glob`, so with a non-local `stores.blob` backend (s3/gcs/adls) — where self-healing stages the patch body — it reported "No pending patches" even though a patch was pending. It now queries the backend-blind `patch_index` table (the Phase-53 relational truth, already used by the heal cache) via the configured observability store, listing pending/applied/rejected regardless of where the bodies live, and points the actions at the backend-aware `patch pull`. Falls back to the local-dir scan when no index/store is reachable (or `--patches-dir` is given). New `patch.index.list_by_status` query. (`aqueduct/cli/patch.py`, `aqueduct/patch/index.py`) **Note:** the other patch-lifecycle commands (`apply`/`discard`/`commit`/`reject`/`import`/`log`) are still local-dir-bound — use `patch pull` to fetch a remote body first; broader backend-awareness is tracked in `TODOs.md`.
