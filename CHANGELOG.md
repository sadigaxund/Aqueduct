...
### Added
- **`@aq.meta.*` — pipeline identity & deployment context Tier-1 functions.** New runtime functions resolvable in any Blueprint/config value: `@aq.meta.blueprint_id()`, `blueprint_name()`, `blueprint_dir()`, `blueprint_path()`, `env()` (`deployment.env`), `target()` (`deployment.target`), and `version()` (engine version). `blueprint_dir()` is the reproducible "relative-to-this-pipeline" path anchor (e.g. `path: @aq.meta.blueprint_dir()/out`). Deliberately omits `cwd`/user/host — those differ across laptop ↔ CI ↔ driver ↔ cluster and would make a Blueprint non-reproducible. Threaded from the compiler (`blueprint_id`/`name`/`path` always available; `env`/`target` from the run config); resolved pre-job on the driver like every other `@aq.*` call, with full provenance tagging. (`aqueduct/compiler/runtime.py`, `aqueduct/compiler/compiler.py`, `aqueduct/cli/run.py`, `docs/specs.md`, `SKILL.md`)
