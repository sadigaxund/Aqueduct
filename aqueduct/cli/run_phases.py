...
class RunContext:
    ...
    # ── fields added for `run_heal_loop` (Phase 91 commit 2) ──────────────
    # CLI params the heal loop reads directly.
    resume_run_id: str | None = None
    from_module: str | None = None
    to_module: str | None = None
    parallel: bool = False
    profile: str | None = None
    # `_load_engine_config`/`_do_compile` outputs the loop reads.
    probe_sampling: object = None
    cli_overrides: dict | None = None
    depot_reads: object = None
    # Path anchors (phase A).
    project_root: Path | None = None
    blueprint_abs: Path | None = None
    # `_setup_surveyor` outputs the loop reads.
    patches_dir: object = None
    approval_mode: str = ""
    max_patches: int = 0
    patch_store: object = None
    resolved_agent_provider: str | None = None
    resolved_agent_base_url: str | None = None
    resolved_agent_model: str | None = None
    resolved_agent_provider_options: dict | None = None
    resolved_agent_timeout: float | None = None
    resolved_agent_max_reprompts: int | None = None
    resolved_agent_api_key: str | None = None
    resolved_agent_engine_prompt_context: str | None = None
    resolved_agent_blueprint_prompt_context: str | None = None
    resolved_agent_cascade: list | None = None
    resolved_sandbox_master_url: str | None = None


@dataclass
class HealLoopResult:
    """Outputs of `run_heal_loop` that `run()` reads after the loop ends."""
...
def run_heal_loop(ctx: RunContext) -> HealLoopResult:
    """The self-healing run loop — one baseline execute, then (on failure)
    the chained/cascade multi-patch heal `while patch_count < max_patches`.

    Moved verbatim from `run()` (Phase 91 decomposition, commit 2): the
    outer `while True:` and inner `while patch_count < max_patches:` /
    `else:` control flow, every `break`/`continue`, and the 5 closures
    defined inside it (`_on_token`, `_close_stream`, `_on_attempt`,
    `_apply_cb`, `_chain_on_attempt`) are unchanged — see the "LOOP-BOUND
    INVARIANT" and "Loop exited via ..." comments inline for why this
    control flow is load-bearing and must not be restructured.

    Outer locals the loop reads are aliased from `ctx` below so the body
    itself stays byte-identical to its previous home inside `run()` (the
    same technique `render_module_summary` uses). `_execute_target` /
    `_render_module_summary` / `_fire_heal_hook` /
    `_announce_polyglot_sandbox_unavailable` are rebound here the same
    way `run()` used to bind them, via `functools.partial(..., ctx)`.
    """
    import functools
    ...
    engine = ctx.engine
    verbosity = ctx.verbosity
    resolved_store_dir = ctx.resolved_store_dir
    checkpoint_root_abs = ctx.checkpoint_root_abs
    surveyor = ctx.surveyor
    depot = ctx.depot
    bundle = ctx.bundle
    run_id = ctx.run_id
    resume_run_id = ctx.resume_run_id
    from_module = ctx.from_module
    to_module = ctx.to_module
    parallel = ctx.parallel
    probe_sampling = ctx.probe_sampling
    approval_mode = ctx.approval_mode
    max_patches = ctx.max_patches
    resolved_agent_provider = ctx.resolved_agent_provider
    resolved_agent_base_url = ctx.resolved_agent_base_url
    resolved_agent_model = ctx.resolved_agent_model
    resolved_agent_provider_options = ctx.resolved_agent_provider_options
    resolved_agent_timeout = ctx.resolved_agent_timeout
    resolved_agent_max_reprompts = ctx.resolved_agent_max_reprompts
    resolved_agent_api_key = ctx.resolved_agent_api_key
    resolved_agent_engine_prompt_context = ctx.resolved_agent_engine_prompt_context
    resolved_agent_blueprint_prompt_context = ctx.resolved_agent_blueprint_prompt_context
    resolved_agent_cascade = ctx.resolved_agent_cascade
    resolved_sandbox_master_url = ctx.resolved_sandbox_master_url
    patches_dir = ctx.patches_dir
    _obs_store = ctx.obs_store
    _patch_store = ctx.patch_store
    _project_root = ctx.project_root
    blueprint_abs = ctx.blueprint_abs
    profile = ctx.profile
    ...
    from aqueduct.cli.run import _print_gate_ladder
    from aqueduct.executor.models import ExecutionStatus
    from aqueduct.models import ModuleType
    ...
    ctx.handoff_info = {m.id: m.config for m in manifest.modules if m.type == ModuleType.Handoff}
    ...
    if _obs_store is not None and manifest.blueprint_id:
        try:
            with _obs_store.connect() as _pending_cur:
                ...
                # No `ensure_schema` here: `_setup_surveyor` above already
                # opened this same store and ran `patch_index`'s DDL as
                # part of `Surveyor.start()` — a one-time, open-time step.
                # Re-running DDL from this second connection would risk
                # blocking forever against the Surveyor's still-open
                # write transaction (see the 4a347374 postmortem); it is
                # also simply redundant.
    ...
    while True:
        # LOOP-BOUND INVARIANT: every path through this body either
        # `break`s or reaches the unconditional `break` that ends the heal
        # block below, and every LLM diagnosis call spends one unit of
        # `max_patches` (`patch_count += 1`, win or lose). The guard at the
        # top (`patch_count >= max_patches`) is therefore a belt-and-braces
        # second bound, not the only one. Do not add a `continue` — or a
        # branch that falls off the end of the body — without a
        # `patch_count` increment: that is exactly the defect that made an
        # unreachable cascade re-execute the pipeline forever (2.3.0).
        #
        ...
        try:
            ...
            _hints = tuple(f"{r.name} — {r.detail}" for r in _dr if r.status in ("warn", "fail"))
            if _hints:
        ...
        # ── Chained (progressive) multi-patch healing ────────────────
        ...
        #
        # 2.3.0 fix — the multi-model cascade (Phase 44) runs through
        # THIS SAME loop. The Phase 92 fold (2991279) had left it as a
        # bare `if _cascade_tiers: generate_cascade_patch(...)` with no
        # post-agent handling whatsoever: no `patch_count` increment, no
        # `break`. Control fell off the end of the outer `while True:`
        # and re-executed the whole pipeline, forever, whenever the
        # cascade could not produce a patch (all tiers unreachable / no
        # credentials). Cascade remains excluded from CHAINING — one
        # tier patch per attempt, never folded into
        # `accumulated_patches` — but it is now bounded by the same
        # `max_patches` budget as every other heal.
        from aqueduct.agent import merge_patch_specs

        _is_cascade = bool(_cascade_tiers)
        ...
        while patch_count < max_patches:
            ...
            if _is_cascade:
                # No second header for cascade: the `◆ self-healing …
                # cascade · tier 1/N` line printed above already announced
                # this attempt, and tier escalation prints its own `├─`
                # nodes inside the transcript.
                pass
            elif len(accumulated_patches) == 0:
                click.echo(
                    click.style(
            ...
            if _is_cascade:
                ...
                # Phase 44 multi-model cascade. Excluded from chaining
                # (see the loop preamble): it diagnoses `current_failure`,
                # which for cascade never advances past this outer
                # iteration's failure.
                agent_result = generate_cascade_patch(
                    ...
                    # default. Identical to `engine` for a single-engine
                    # run (`failure_ctx.engine` is always that same value
                    # there).
                    engine=(current_failure.engine or engine),
            ...
            # result: the winning cascade tier's model for a cascade,
            # the top-level agent.model otherwise.
            ...
            if effective_mode == "human":
                ...
                _g2, _g3, _g4, _g3_passed = _aqcli._run_patch_gates_inline(
                    ...
                    warnings_suppress=cfg.warnings.suppress,
                    timezone=cfg.timezone,
                    depot_reads_at_failure=ctx.depot_reads,
                )
                ...
                if _g3 is not None and not _g3_passed:
                    ...
                    last_apply_error = (
                        f"Patch {_combined_candidate.patch_id!r} rejected by sandbox: {_g3.detail}"
                    )
                    surveyor.record_healing_outcome(
                        run_id=iteration_run_id,
                        failed_module=current_failure.failed_module,
                ...
                if patch_success:
                    ...
                    if (
                        not _is_cascade
                        and failure_ctx2 is not None
                        and _next_module != _this_module
                    ):
        ...
        # was the exact bug the Phase 92 fold fixed for chaining: an outer
        # `continue` used to throw away every accumulated link and restart
        # a fresh chain; and the exact bug it INTRODUCED for cascade: no
        # handling at all, so control fell through to here-minus-the-break
        # and looped forever). Always end the run's outer loop here,
        # regardless of how the heal ended.
        break

    return HealLoopResult(
        result=result,
        failure_ctx=failure_ctx,
