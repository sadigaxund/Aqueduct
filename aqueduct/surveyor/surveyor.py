...
if TYPE_CHECKING:
    from aqueduct.stores import ObsStore, StoreBundle
...
class Surveyor:
    ...
    def __init__(
        ...
        self._obs: "ObsStore | None" = stores.obs if stores is not None else None
        self._started: bool = False  # DDL/migrations applied once per Surveyor.start()

    # ── Lifecycle ─────────────────────────────────────────────────────────────

    def start(self, run_id: str) -> None:
        """Open the configured obs store and record run start.

        Creates parent directories for DuckDB-backed stores; for Postgres
        the `obs` schema is auto-created on the first `connect()`.
        ...
        if self._obs is None:
            ...
            self._obs = DuckDBObsStore(self._store_dir / "obs.db")
    ...
    def record(
        ...
        """
        if not self._started or self._obs is None or self._run_id is None:
    ...
    def evaluate_regulator(self, regulator_id: str) -> bool:
        ...
        """
        if self._run_id is None or self._obs is None:
    ...
    def get_probe_signal(
        ...
        if self._obs is None:
            from aqueduct.stores.duckdb_ import DuckDBObsStore
            obs_path = self._store_dir / "obs.db"
            if not obs_path.exists():
                return []
            self._obs = DuckDBObsStore(obs_path)
