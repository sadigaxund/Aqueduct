...
def test_funnel_unknown_input(dfs):
    module = Module(
        label="M", id="f1", type="Funnel", config={"mode": "union_all", "inputs": ["m1", "ghost"]}
    )
...
def test_union_all_strict(dfs):
    module = Module(
        label="M", id="f1", type="Funnel", config={"mode": "union_all", "inputs": ["m1", "m2"]}
    )
...
def test_union_all_strict_mismatched(dfs):
    module = Module(
        label="M", id="f1", type="Funnel", config={"mode": "union_all", "inputs": ["m1", "m3"]}
    )
...
def test_union(dfs):
    ...
    module = Module(
        label="M", id="f1", type="Funnel", config={"mode": "union", "inputs": ["m1", "m2"]}
    )
    res = execute_funnel(module, dfs)
    assert res.count() == 2
...
def test_coalesce_row_aligned(spark):
    dfa = spark.sql(
        "SELECT 1 as id, CAST(NULL as STRING) as c1, 'a' as c2 UNION ALL SELECT 2, 'b', NULL"
    )
