...
def independent_manifest(tmp_path):
    ...
    m_a = Module(
        ...
        type="Ingress",
        label="A",
        config={"path": str(tmp_path / "a.parquet"), "format": "parquet"},
    )
    m_b = Module(
        id="B",
        type="Egress",
        label="B",
        config={"path": str(tmp_path / "b.parquet"), "format": "parquet"},
    )
    m_c = Module(
        id="C",
        type="Ingress",
        label="C",
        config={"path": str(tmp_path / "c.parquet"), "format": "parquet"},
    )
    m_d = Module(
        id="D",
        type="Egress",
        label="D",
        config={"path": str(tmp_path / "d.parquet"), "format": "parquet"},
    )
...
def test_execute_parallel_unexpected_thread_exception(
    independent_manifest, tmp_path, spark, monkeypatch
):
