...
def test_delta_ops_optimize_and_vacuum():
    ops = build_maintenance_ops(
        "delta", "/t", None, {"optimize": True, "zorder_by": "id", "vacuum": 168}
    )
...
def test_iceberg_ops_use_catalog_procedures():
    ops = build_maintenance_ops(
        "iceberg", "", "local.db.orders", {"rewrite_data_files": True, "expire_snapshots": True}
    )
    labels = {label: sql for _slot, label, sql in ops}
    assert (
        labels["rewrite_data_files"] == "CALL local.system.rewrite_data_files(table => 'db.orders')"
    )
...
def test_iceberg_without_catalog_table_raises():
    ...
    with pytest.raises(EgressError, match="catalog table"):
        build_maintenance_ops(
            "iceberg", "", "db.tbl", {"rewrite_data_files": True}
        )  # not fully-qualified
