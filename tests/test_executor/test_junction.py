...
def test_execute_junction_empty_branches(df):
    module = Module(
        label="M", id="j1", type="Junction", config={"mode": "broadcast", "branches": []}
    )
...
def test_execute_junction_branch_missing_id(df):
    module = Module(
        ...
        id="j1",
        type="Junction",
        config={"mode": "broadcast", "branches": [{"condition": "x"}]},
    )
...
def test_conditional_missing_condition(df):
    module = Module(
        ...
        id="j1",
        type="Junction",
        config={"mode": "conditional", "branches": [{"id": "b1"}]},
    )
...
def test_conditional_else_only(df):
    cfg = {"mode": "conditional", "branches": [{"id": "fallback", "condition": "_else_"}]}
    module = Module(label="M", id="j1", type="Junction", config=cfg)
    res = execute_junction(module, df)
...
def test_broadcast(df):
    cfg = {"mode": "broadcast", "branches": [{"id": "b1"}, {"id": "b2"}]}
    module = Module(label="M", id="j1", type="Junction", config=cfg)
    res = execute_junction(module, df)
...
def test_partition_missing_key(df):
    cfg = {"mode": "partition", "branches": [{"id": "US"}]}
    module = Module(label="M", id="j1", type="Junction", config=cfg)
...
def test_partition_fallback_to_id(df):
    cfg = {"mode": "partition", "partition_key": "region", "branches": [{"id": "US"}, {"id": "DE"}]}
    module = Module(label="M", id="j1", type="Junction", config=cfg)
    res = execute_junction(module, df)
