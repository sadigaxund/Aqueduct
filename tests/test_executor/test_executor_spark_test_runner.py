...
def test_assertion_sql(spark: SparkSession):
    ...
    # Fail (empty result)
    res = _run_assertion(
        {"type": "sql", "expr": "SELECT * FROM __output__ WHERE id > 100"}, df, spark
    )
    assert res.passed is False
...
def test_execute_module_dispatch(spark: SparkSession):
    # Channel
    m = Module(
        id="m1", type="Channel", label="L", config={"op": "sql", "query": "SELECT * FROM in1"}
    )
    in1 = spark.range(3)
    ...
    # Assert
    m_assert = Module(
        id="a1", type="Assert", label="L", config={"rules": [{"type": "sql_row", "expr": "id > 0"}]}
    )
    res_assert = _execute_module(
        m_assert, {"in": spark.createDataFrame([(0,), (1,)], ["id"])}, spark
    )
...
def test_run_test_case_junction(spark: SparkSession):
    ...
    m = Module(
        ...
        config={
            "mode": "conditional",
            "branches": [{"id": "b1", "condition": "id > 0"}, {"id": "b2", "condition": "id <= 0"}],
        },
    )
    test_case = {
        "id": "tj",
        "module": "j1",
        "inputs": {"in": {"schema": {"id": "int"}, "rows": [[1], [0]]}},
...
def test_bundled_template_conformance(spark: SparkSession, tmp_path: Path):
    ...
    assert (
        raw.get("aqueduct_test") == "1.0"
    ), f"Unexpected aqueduct_test version: {raw.get('aqueduct_test')}"
    ...
    for idx, test_case in enumerate(tests):
        ...
        assert (
            isinstance(test_case.get("module"), str) and test_case["module"]
        ), f"Test {test_id} missing non-empty 'module' string"

        inputs = test_case.get("inputs")
        assert (
            isinstance(inputs, dict) and inputs
        ), f"Test {test_id} inputs must be a non-empty mapping"

        for input_id, input_spec in inputs.items():
            assert (
                isinstance(input_spec.get("schema"), dict) and input_spec["schema"]
            ), f"Input {input_id} in {test_id} must have a non-empty 'schema' dict"
            assert isinstance(
                input_spec.get("rows"), list
            ), f"Input {input_id} in {test_id} must have a 'rows' list"

        assertions = test_case.get("assertions")
        assert (
            isinstance(assertions, list) and assertions
        ), f"Test {test_id} must have a non-empty assertions list"
        for a_idx, assertion in enumerate(assertions):
            assert assertion.get("type") in {
                "row_count",
                "contains",
                "sql",
            }, f"Assertion {a_idx} in {test_id} has invalid/unknown type {assertion.get('type')}"
