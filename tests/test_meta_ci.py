...
_SPARK_IMPORT_ALLOWLIST: dict[str, str] = {
    ...
    "aqueduct/cli/run_setup.py": (
        ...
        "_PS` TYPE_CHECKING-only annotation import moved here verbatim from "
        "`aqueduct/cli/run.py` (no behaviour change). The TYPE_CHECKING "
        "import is annotation-only, never executed. The runtime "
        "ProbeSampling() construction is NOT engine-gated (pre-existing — "
        "every engine gets a Spark-flavored ProbeSampling config object "
        "here today); flagging honestly rather than retrofitting a guard, "
        "since this split must be behaviour-identical. Worth a follow-up "
        "to give DuckDB its own ProbeSampling wiring (it already has its "
        "own `ProbeSampling` class in `aqueduct/executor/duckdb_/probe.py` "
        "that this code path never uses)."
    ),
