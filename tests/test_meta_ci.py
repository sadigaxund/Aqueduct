...
_MAIN_ONLY_TEST_DIR_ALLOWLIST: dict[str, str] = {
    ...
    "tests/test_deploy": (
        "Phase 86: the Databricks deploy layer (`aqueduct/deploy/`, its "
        "`databricks` extra) was deleted on the group branch this worktree "
        "will merge into, and its `deploy-tests` pre-merge job was removed "
        "here as dead CI (the job title's own directory no longer exists on "
        "that branch). This worktree's base predates that deletion, so "
        "`tests/test_deploy/` is still physically present and would "
        "otherwise trip this guard as an unreferenced directory. Not "
        "reintroducing a pre-merge lane for a directory that is already "
        "gone upstream; this allowlist entry (and the directory itself) "
        "disappear together on the next merge from the group branch."
    ),
}
