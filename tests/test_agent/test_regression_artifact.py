        ...
        operations=[
            {
                ...
                "module_id": "clean_orders",
                "key": "query",
                "value": "SELECT * FROM raw_orders",
            }
        ],
    )
    base.update(overrides)
...
def _channel_with_upstream(
    schema_hint: list[dict] | None = "MISSING",
    ...
    channel = Module(
        ...
        type="Channel",
        label="clean",
        config={"op": "sql", "query": "SELECT * FROM raw_orders"},
    )
...
class TestGenerateHappyPath:
    def test_writes_valid_aqtest_yaml(self, tmp_path: Path):
        ...
        assert test_case["inputs"]["raw_orders"]["schema"] == {
            "order_id": "bigint",
            "amount": "double",
        }
    ...
    def test_never_overwrites_existing_file(self, tmp_path: Path):
        modules, edges = _channel_with_upstream(
            schema_hint=[{"name": "order_id", "type": "bigint"}]
        )
        manifest = _manifest(modules, edges)
        blueprint_path = tmp_path / "blueprint.yml"
        blueprint_path.write_text("aqueduct: 1.0\n")
        ...
        second = generate(
            manifest, _patch(patch_id="fix-amount-type"), _failure_ctx(), blueprint_path
        )
...
class TestGenerateConservativeSkips:
    ...
    def test_skip_when_module_type_not_testable(self, tmp_path: Path):
        ingress = Module(
            id="raw_orders", type="Ingress", label="raw", config={"format": "parquet", "path": "p"}
        )
    ...
    def test_skip_when_schema_hint_type_unmapped(self, tmp_path: Path):
        modules, edges = _channel_with_upstream(
            schema_hint=[{"name": "geo", "type": "array<string>"}]
        )
        manifest = _manifest(modules, edges)
        blueprint_path = tmp_path / "blueprint.yml"
        blueprint_path.write_text("aqueduct: 1.0\n")
