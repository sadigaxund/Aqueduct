...
def _stamp(store, **kw):
    defaults = dict(
        patch_id="p1",
        object_key="/obj/p1.json",
        blueprint_id="bp1",
        status="pending",
        source="llm",
        signature="abc123",
        signature_coarse="abc12345",
        error_class="E1",
        where_field="m1",
        normalized_message="boom",
        rationale="fix",
        ops='[{"op": "set_module_config_key"}]',
        engine="spark",
    )
    defaults.update(kw)
...
class TestFindPending:
    ...
    def test_newest_match_wins(self, tmp_path):
        ...
        store = _make_store(tmp_path / "obs.db")
        _stamp(store, patch_id="old", signature="abc123", created_at="2020-01-01 00:00:00")
        _stamp(store, patch_id="new", signature="abc123", created_at="2025-01-01 00:00:00")
        hit = find_pending(store, "abc123")
        assert hit is not None
...
class TestCoachingExampleDefaults:
    def test_tier_defaults_to_4(self):
        ...
        ex = CoachingExample(
            patch_id="p1", error_class="E", where="m1", normalized_message="x", rationale="fix"
        )
        assert ex.tier == 4
        assert ex.ops == []
