...
def test_obs_store_roundtrip(obs_store):
    with obs_store.connect() as cur:
        ...
        assert (
            cur.execute("SELECT val FROM run_records WHERE run_id=?", ["run_1"]).fetchone()[0]
            == "ok"
        )
        ...
        assert (
            cur.execute("SELECT val FROM module_metrics WHERE module_id=?", ["mod_1"]).fetchone()[0]
            == 42
        )
        ...
        assert (
            cur.execute(
                "SELECT passed FROM probe_signals WHERE probe_id=?", ["probe_1"]
            ).fetchone()[0]
            is True
        )
