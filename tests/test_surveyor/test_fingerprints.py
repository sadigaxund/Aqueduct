...
def test_unchanged_sql_keeps_one_row_and_bumps_last_run(tmp_path):
    ...
    write_fingerprints(
        "bp.x", "run2", (_ch("select a from t where x=1  -- c"),), observability_store=obs
    )
...
def test_semantic_change_appends_a_row(tmp_path):
    obs = _store(tmp_path)
    write_fingerprints(
        "bp.x", "run1", (_ch("SELECT a FROM t WHERE x = 1"),), observability_store=obs
    )
    write_fingerprints(
        "bp.x", "run2", (_ch("SELECT a FROM t WHERE x = 2"),), observability_store=obs
    )
