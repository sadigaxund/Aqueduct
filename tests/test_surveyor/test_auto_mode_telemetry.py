...
def test_surveyor_record_on_conflict(manifest, tmp_path):
    ...
    conn = duckdb.connect(str(tmp_path / "observability.db"))
    assert (
        conn.execute("SELECT status FROM run_records WHERE run_id = ?", [run_id]).fetchone()[0]
        == "error"
    )
...
def test_heal_attempts_no_duplicate_on_stop_reason(manifest, tmp_path):
    ...
    surveyor.record_heal_attempt(
        run_id="run-attempts", attempt_record=mock_record, stop_reason=None
    )
    ...
    surveyor.update_heal_attempt_stop_reason(
        run_id="run-attempts", attempt_num=1, stop_reason=StopReason.SOLVED
    )
...
def test_heal_attempts_chain_link_column_migration_on_existing_db(manifest, tmp_path):
    ...
    chain_link) must gain the column on Surveyor init — CREATE TABLE IF NOT
    EXISTS alone never adds columns to an existing table, so without the
    ALTER migration every heal-attempt INSERT fails silently on upgraded
    installs."""
    conn = duckdb.connect(str(tmp_path / "observability.db"))
    conn.execute(
        """
    ...
    surveyor.record_heal_attempt(
        run_id="run-chain-migration", attempt_record=mock_record, stop_reason=None
    )
