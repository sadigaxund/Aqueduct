...
def _seed(store_dir, module_results):
    ...
    c.execute("INSERT INTO module_metrics VALUES ('r1','heavy',NULL,NULL,1000,999999,5000,now())")
    c.execute("INSERT INTO module_metrics VALUES ('r1','light',NULL,NULL,10,100,50,now())")
    c.close()
...
def test_html_report_renders(tmp_path):
    _seed(
        tmp_path,
        [
            {"module_id": "heavy", "status": "success", "error": ""},
            {"module_id": "light", "status": "success", "error": ""},
        ],
    )
    res = CliRunner().invoke(
        cli, ["report", "r1", "--format", "html", "--store-dir", str(tmp_path)]
    )
...
def test_html_report_no_metrics_ok(tmp_path):
    ...
    c.execute(_DDL)
    c.execute(
        "INSERT INTO run_records VALUES ('r1','bp','success', now(), now(), ?)",
