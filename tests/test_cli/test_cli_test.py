...
"""
    ...
    result = runner.invoke(
        cli, ["test", str(test_setup / "pass.aqtest.yml"), "--blueprint", str(bp2)]
    )
    from aqueduct.exit_codes import DATA_OR_RUNTIME
...
"""
    ...
    with (
        patch("aqueduct.executor.spark.session.make_spark_session") as mock_make,
        patch(
            "aqueduct.executor.spark.test_runner.run_test_file",
            return_value=TestSuiteResult(total=1, passed=1, failed=0, results=[]),
        ),
    ):

        runner = CliRunner()
        result = runner.invoke(
            cli, ["test", str(test_setup / "pass.aqtest.yml"), "--config", str(cfg_path)]
        )

        # Check stderr notice was printed because master_url is non-local and no --master was passed
        assert (
            "(test: ignoring engine.spark.master_url='spark://h:7077'; running on local[*] — pass --master to override)"
            in result.output
        )
