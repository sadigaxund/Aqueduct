...
def mock_agent_patch():
    def _make_patch(patch_id="fix-1", rationale="test", ops=None):
        return PatchSpec(
            ...
            root_cause="test",
            operations=ops
            or [{"op": "replace_module_label", "module_id": "in", "label": "New Label"}],
        )
...
def test_trigger_agent_escalation(
    ...
    base_blueprint.write_text(
        base_blueprint.read_text().replace("approval: auto", "approval: disabled")
    )
...
def test_trigger_agent_false_disabled_breaks(
    ...
    mock_exec.return_value = ExecutionResult(
        blueprint_id="test_bp", run_id="r1", status="error", module_results=(), trigger_agent=False
    )
...
def test_block_full_actions_propagation(
    ...
    mock_cfg = AqueductConfig(
        danger=DangerConfig(allow_full_probe_actions=False, allow_multi_patch=True),
...
def test_trigger_agent_stays_human(
    ...
    # Set to human
    base_blueprint.write_text(
        base_blueprint.read_text().replace("approval: auto", "approval: human")
    )
