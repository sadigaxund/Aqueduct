...
class TestDoctorFormatMismatch:
    def test_parquet_with_parquet_ok(self):
        ...
        _check_format_ext_mismatch(
            results, "ingress:m1", "parquet", ["data.parquet"], "/tmp/data.parquet", 100
        )
    ...
    def test_csv_with_parquet_warns(self):
        results = []
        _check_format_ext_mismatch(
            results, "ingress:m1", "csv", ["data.parquet"], "/tmp/data.parquet", 100
        )
    ...
    def test_parquet_with_csv_warns(self):
        results = []
        _check_format_ext_mismatch(
            results, "ingress:m1", "parquet", ["data.csv"], "/tmp/data.csv", 100
        )
        assert len(results) == 1
        assert results[0].status == "warn"
    ...
    def test_delta_no_mismatch_check(self):
        results = []
        _check_format_ext_mismatch(
            results, "ingress:m1", "delta", ["data.csv", "data.parquet", "data"], "/tmp/data", 100
        )
        assert len(results) == 0
    ...
    def test_unknown_format_no_check(self):
        results = []
        _check_format_ext_mismatch(
            results, "ingress:m1", "weird_format", ["data.csv"], "/tmp/data.csv", 100
        )
        assert len(results) == 0
    ...
    def test_glob_with_mixed_extensions_warns(self):
        results = []
        _check_format_ext_mismatch(
            results, "ingress:m1", "parquet", ["data1.parquet", "data2.csv"], "/tmp/data*", 100
        )
        assert len(results) == 1
        assert results[0].status == "warn"
...
class TestDoctorAQTest:
    ...
    def test_check_aqtest_blueprint_not_found(self, tmp_path):
        ...
        results = check_aqtest(p)
        assert any(
            r.status == "fail" and "does not resolve to an existing file" in r.detail
            for r in results
        )
    ...
    def test_check_aqtest_module_not_in_blueprint(self, tmp_path):
        ...
        p.write_text(
            "aqueduct_test: '1.0'\nblueprint: bp.yml\ntests: [{id: t1, module: m2, assertions: []}]",
            encoding="utf-8",
        )
        results = check_aqtest(p)
        assert any(
            r.status == "fail" and "module='m2' not in blueprint" in r.detail for r in results
        )
    ...
    def test_check_aqtest_ok(self, tmp_path):
        ...
        p.write_text(
            "aqueduct_test: '1.0'\nblueprint: bp.yml\ntests: [{id: t1, module: m1, assertions: [{type: row_count, min: 1}]}]",
            encoding="utf-8",
        )
        results = check_aqtest(p)
...
class TestDoctorAQScenario:
    ...
    def test_check_aqscenario_module_mismatch(self, tmp_path):
        ...
        p.write_text(
            "aqueduct_scenario: '1.0'\nid: s1\nblueprint: bp.yml\ninject_failure: {module: m2}",
            encoding="utf-8",
        )
        results = check_aqscenario(p)
        assert any(
            r.status == "fail" and "inject_failure.module='m2' not in blueprint" in r.detail
            for r in results
        )
...
class TestDoctorCLIFlags:
    ...
    def test_doctor_additive_flags_all_reported(self, tmp_path):
        ...
        result = runner.invoke(
            cli, ["doctor", str(bp), "--aqtest", str(aqt), "--aqscenario", str(aqs), "--skip-spark"]
        )
