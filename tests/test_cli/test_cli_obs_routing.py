...
def test_runs_command_unions_all_databases(tmp_path):
    ...
    conn1.execute(
        "CREATE TABLE run_records (run_id VARCHAR PRIMARY KEY, blueprint_id VARCHAR, status VARCHAR, started_at TIMESTAMPTZ, finished_at TIMESTAMPTZ, module_results JSON)"
    )
    conn1.execute(
        "INSERT INTO run_records VALUES ('run-1', 'pipeline_1', 'success', '2026-05-26T10:00:00Z', '2026-05-26T10:00:05Z', '[]')"
    )
    conn1.close()
    ...
    conn2.execute(
        "INSERT INTO run_records VALUES ('run-2', 'pipeline_2', 'error', '2026-05-26T11:00:00Z', '2026-05-26T11:00:05Z', '[]')"
    )
    conn2.close()
...
def test_heal_command_per_pipeline_routing(tmp_path):
    ...
    manifest = Manifest(
        blueprint_id="pipeline_1", modules=(), edges=(), context={}, engine_config={}
    )
