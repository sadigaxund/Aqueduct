...
def test_cwd_restoration_after_run(tmp_path):
    ...
    project_dir.mkdir()
    (project_dir / "blueprint.yml").write_text(
        "aqueduct: '1.0'\nid: repro\nname: Repro\nmodules: []\nedges: []"
    )
    (project_dir / "aqueduct.yml").write_text(
        "deployment:\n  engine: spark\nengine:\n  spark:\n    master_url: local[1]\n"
    )
    ...
    try:
        ...
        final_cwd = os.getcwd()
        assert (
            final_cwd == initial_cwd
        ), f"CWD was not restored! Expected {initial_cwd}, got {final_cwd}"
...
def test_cwd_restoration_after_failure(tmp_path):
    ...
    try:
        ...
        assert (
            final_cwd == initial_cwd
        ), f"CWD was not restored after failure! Expected {initial_cwd}, got {final_cwd}"
