...
def obs_db(tmp_path):
    ...
    conn.execute(
        "CREATE TABLE run_records (run_id VARCHAR, blueprint_id VARCHAR, status VARCHAR, started_at TIMESTAMP, finished_at TIMESTAMP, module_results JSON)"
    )
    conn.execute(
        "CREATE TABLE failure_contexts (run_id VARCHAR PRIMARY KEY, blueprint_id VARCHAR, failed_module VARCHAR, error_message VARCHAR, stack_trace VARCHAR, manifest_json VARCHAR, provenance_json VARCHAR, started_at TIMESTAMP, finished_at TIMESTAMP)"
    )
...
def lineage_obs_db(tmp_path):
    ...
    conn = duckdb.connect(str(db_path))
    conn.execute(
        "CREATE TABLE column_lineage (blueprint_id VARCHAR, run_id VARCHAR, channel_id VARCHAR, output_column VARCHAR, source_table VARCHAR, source_column VARCHAR)"
    )
    conn.execute(
        "INSERT INTO column_lineage VALUES ('bp1', 'run1', 'chan1', 'col1', 'src1', 'scol1')"
    )
    conn.close()
    return store_dir
...
class TestReportCommand:
    def test_report_valid_run(self, cli_runner, obs_db, aq_config):
        ...
        conn.execute(
            "INSERT INTO run_records VALUES ('run1', 'bp1', 'success', '2026-01-01 10:00:00', '2026-01-01 10:05:00', '[]')"
        )
        conn.close()
...
class TestSignalCommand:
    def test_signal_set_override(self, cli_runner, obs_db, aq_config):
        ...
        conn = duckdb.connect(str(obs_db))
        row = conn.execute(
            "SELECT passed FROM signal_overrides WHERE signal_id = 'probe1'"
        ).fetchone()
        conn.close()
    ...
    def test_signal_clear_override(self, cli_runner, obs_db, aq_config):
        conn = duckdb.connect(str(obs_db))
        conn.execute(
            "INSERT INTO signal_overrides VALUES ('probe1', false, 'reason', '2026-01-01')"
        )
        conn.close()
        result = cli_runner.invoke(
            cli,
        ...
        conn = duckdb.connect(str(obs_db))
        count = conn.execute(
            "SELECT COUNT(*) FROM signal_overrides WHERE signal_id = 'probe1'"
        ).fetchone()[0]
        conn.close()
        assert count == 0
    ...
    def test_close_gate_with_error_msg(self, cli_runner, obs_db, aq_config):
        ...
        conn = duckdb.connect(str(obs_db))
        row = conn.execute(
            "SELECT passed, error_message FROM signal_overrides WHERE signal_id = 'my_probe'"
        ).fetchone()
        conn.close()
        assert row[0] is False
    ...
    def test_no_flags_shows_status(self, cli_runner, aq_config):
        result = cli_runner.invoke(
            cli, ["signal", "my_probe", "--blueprint", "bp", "--config", str(aq_config)]
        )
        assert result.exit_code == 0
...
class TestRunsCommand:
    ...
    def test_runs_list_ordered(self, cli_runner, obs_db, aq_config):
        ...
        conn = duckdb.connect(str(obs_db))
        conn.execute(
            "INSERT INTO run_records VALUES ('r1', 'bp1', 'success', '2026-01-01 10:00:00', '2026-01-01 10:01:00', '[]')"
        )
        conn.execute(
            "INSERT INTO run_records VALUES ('r2', 'bp1', 'success', '2026-01-01 11:00:00', '2026-01-01 11:01:00', '[]')"
        )
        conn.close()
    ...
    def test_runs_failed_filter(self, cli_runner, obs_db, aq_config):
        ...
        conn = duckdb.connect(str(obs_db))
        conn.execute(
            "INSERT INTO run_records VALUES ('r_ok', 'bp1', 'success', '2026-01-01 10:00:00', '2026-01-01 10:01:00', '[]')"
        )
        conn.execute(
            "INSERT INTO run_records VALUES ('r_err', 'bp1', 'error', '2026-01-01 11:00:00', '2026-01-01 11:01:00', '[]')"
        )
        conn.close()
    ...
    def test_runs_blueprint_filter(self, cli_runner, obs_db, aq_config, tmp_path):
        ...
        conn = duckdb.connect(str(obs_db))
        conn.execute(
            "INSERT INTO run_records VALUES ('r_target', 'target_bp', 'success', '2026-01-01 10:00:00', '2026-01-01 10:01:00', '[]')"
        )
        conn.execute(
            "INSERT INTO run_records VALUES ('r_other', 'other_bp', 'success', '2026-01-01 11:00:00', '2026-01-01 11:01:00', '[]')"
        )
        conn.close()
        ...
        result = cli_runner.invoke(
            cli, ["runs", "--blueprint", str(bp_file), "--config", str(aq_config)]
        )
        assert result.exit_code == 0
    ...
    def test_runs_limit(self, cli_runner, obs_db, aq_config):
        ...
        conn = duckdb.connect(str(obs_db))
        conn.execute(
            "INSERT INTO run_records VALUES ('r1', 'bp', 'success', '2026-01-01 10:00:00', '2026-01-01 10:01:00', '[]')"
        )
        conn.execute(
            "INSERT INTO run_records VALUES ('r2', 'bp', 'success', '2026-01-01 11:00:00', '2026-01-01 11:01:00', '[]')"
        )
        conn.close()
