...
def test_patch_reject_bare_slug(cli_setup):
    ...
    result = runner.invoke(
        cli, ["patch", "reject", "P001", "--patches-dir", str(patches_dir), "--reason", "Test slug"]
    )
    ...
    def mock_run(args, **kwargs):
        ...
        if "log" in args:
            ...
            return subprocess.CompletedProcess(
                args, 1, stdout="", stderr="fatal: not a git repository"
            )
...
def test_patch_commit_multiple_patches(cli_setup, monkeypatch):
    ...
    patch_data1 = {
        ...
        "rationale": "Fix 1",
        "operations": [
            {"op": "set_module_config_key", "module_id": "src", "key": "path", "value": "a.csv"}
        ],
    }
    patch_file.write_text(json.dumps(patch_data1))
    apply_result1 = runner.invoke(
        cli, ["patch", "apply", str(patch_file), "--blueprint", str(bp_path)]
    )
    ...
    patch_data2 = {
        ...
        "rationale": "Fix 2",
        "operations": [
            {"op": "set_module_config_key", "module_id": "src", "key": "path", "value": "b.csv"}
        ],
    }
    patch_file2.write_text(json.dumps(patch_data2))
    apply_result2 = runner.invoke(
        cli, ["patch", "apply", str(patch_file2), "--blueprint", str(bp_path)]
    )
