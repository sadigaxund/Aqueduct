...
- `aqueduct/executor/session_config.py` — engine-agnostic, **pyspark-free** `SessionSpec` engine-config resolution (Phase 82 remediation): `resolve_session_engine_config(cfg, engine, manifest)` (Spark's merged-conf precedence vs. every other engine's `engine.<name>` sub-model dumped via `model_dump()`) and `session_secrets_options(cfg, manifest)` (the `secrets:` block bag for `SessionSpec.engine_options`). Split out of `protocol.py` on purpose — these are session-config helpers, not part of the `ExecutorProtocol` contract an engine implements, and `protocol.py`'s public surface is the thing tracked for the BYO-engine stability criterion (see the "READINESS TEST" standing decision). Every `SessionSpec` builder resolves through here: `aqueduct/cli/run.py` (single-engine and polyglot) AND `aqueduct/patch/preview.py::run_sandbox_gate` (Gate 3) — so a sandbox replay sees the SAME engine config a real run would use, not `sandboxed_manifest.spark_config` (always `{}` for a non-Spark target, the defect this module's extraction fixed).
...
- `aqueduct/executor/spill.py` — engine-agnostic cross-engine handoff spill directory lifecycle (Phase 81 step 3). The engine-native parquet write/read at a handoff boundary lives in each engine's own executor; this module owns everything AROUND that write/read — where a boundary's spill directory lives, deleting it on a successful run, keeping it on failure (so a rerun after a heal can resume from the upstream island's already-materialized output).
...
**Corrected classification, 2026-07-31: `config.lineage.*` is CORE.** OpenLineage emission (`Surveyor.__init__`, `aqueduct/surveyor/surveyor.py`) is gated only on `openlineage_url` being set, with no engine condition anywhere — a DuckDB run emits OpenLineage events exactly like Spark. Both engines previously carried a verdict for `lineage.openlineage_url`/`openlineage_namespace` (DuckDB's `ignored_with_warning` hint falsely claimed "the DuckDB executor does not emit OpenLineage events"); both rows are now gone from both tables — the leaves left the checklist entirely rather than getting a corrected verdict, because they are genuinely core.

**Justify a classification by a consumer, never by another engine's verdict.** `config.lineage.*` was classified engine-scoped *because* DuckDB declared it `ignored_with_warning` — circular reasoning that treated a verdict as evidence for the classification that produces it. The verdict was itself wrong (its hint claimed the DuckDB executor emits no OpenLineage events; emission is core and always ran on both engines). Note the direction the invariant test fails in: it forbids reclassifying any leaf some engine declares non-`supported`, which protects genuine warning paths **and** wrong verdicts equally. It is a ratchet, not an oracle. So when tagging a field, grep its readers and decide from them; an existing `capabilities.yml` row is a claim to be checked, not a source of truth.
...
| `regression_artifact.py` | `generate`, `RegressionArtifactResult` | Opt-in `agent.regression_artifact` — emits a CI `.aqtest.yml` regression test after a successful heal, guarding against the fix being silently undone later (hand-edit, SQL refactor, revert). Different job from the signature-memory heal cache (`signature.py`/`memory.py`), which instead resolves the SAME failure recurring in production for zero tokens |
...
| `executor-tests` | `aqueduct/executor/**`, `tests/test_executor/**`, `tests/test_compiler/test_blueprints.py`, `gallery/snippets/**`, or a handful of genuinely Spark-dependent files that live outside `tests/test_executor/` (`tests/test_surveyor/test_probe.py`, `tests/test_patch/test_patch_preview_gate3.py`, `tests/test_patch/test_preview_sandbox_explain.py`, `tests/test_sync_constants.py`, `tests/test_executor_duckdb/test_handoff_s3_minio.py`) | Two invocations: `pytest tests/test_executor/ tests/test_compiler/test_blueprints.py <those 5 files> -m spark`, then `pytest tests/test_executor/ -m "not spark"` for the `unit`-marked minority of that directory that the first invocation deselects — this job is the ONLY pre-merge lane pointing at `tests/test_executor/`, so without the second pass those files ran nowhere pre-merge |
